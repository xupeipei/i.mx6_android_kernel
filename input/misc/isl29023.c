...
struct i2c_client *lux_client = NULL;
...
{
	int val = 0;
	val = isl29023_get_int_lt_value(lux_client);
	printk(KERN_INFO "lt lux %d\n",val);

	val = isl29023_get_int_ht_value(lux_client);
	printk(KERN_INFO "ht lux %d\n",val);	
	return val;
}
EXPORT_SYMBOL_GPL(read_lux);
