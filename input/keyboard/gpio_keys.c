...
#define SABRESD_GPS_EN	IMX_GPIO_NR(2, 3)
...
#define RATIO_X (EGALAX_MAX_X / RESOLUTION_X)
#define RATIO_Y (EGALAX_MAX_Y / RESOLUTION_Y)
/*screen position G*/
#define G_X             500
#define G_Y             455
extern input_sw;
...
static void gpio_keys_report_event(struct gpio_button_data *bdata)
{
	...
	static unsigned int gps_flag = 0, lcd_flag = 0, keyboard_flag = 0;
	int state = (gpio_get_value_cansleep(button->gpio) ? 1 : 0) ^ button->active_low;
	int r;
	//printk(KERN_INFO "GPIO PRESS TYPE : %d\n", type);/*test*/
	...
	if (type == EV_ABS) {
		...
		//if (input_sw != NULL) {
			//printk(KERN_INFO "button%d status%d", button->code, state);/*test*/	
			input_event(input, type, button->code, !!state);
			switch (button->code) {
				case KEY_F1: {
					if (request == 0) {
						request = 1;
						r = gpio_request(FM_PWD, "fm2018_pwd");
						if (r < 0) {
							printk(KERN_INFO "fail to requist fm2018 power down gpio!\n");
						}
...
static int __devinit gpio_keys_probe(struct platform_device *pdev)
{
	...
	gpio_set_value(SABRESD_GPS_EN, 1);
	/*lcd*/
	gpio_request(LCD_EN, "lcd_en");
	gpio_direction_output(LCD_EN, 1);
	gpio_set_value(LCD_EN, 1);
	/*keyboard*/
	gpio_request(KEYBOARD_EN, "keyboard_en");
	gpio_direction_output(KEYBOARD_EN, 1);
