...
struct input_dev *input_sw = NULL;/*test*/
...
static irqreturn_t egalax_ts_interrupt(int irq, void *dev_id)
{
	...
	do {
		//printk(KERN_INFO "******************entering tc interrupt\n");
		do {
			//printk(KERN_INFO "recv data from i2c\n");
		...
		//printk(KERN_INFO "input tc id :%d status :%d\n", id, down);/*test*/
				       ...
				       const struct i2c_device_id *id)
{
	...
	/*test*/
	input_sw = kzalloc(sizeof(struct input_dev), GFP_KERNEL);
