...
#define	BATTERY_UPDATE_INTERVAL	10 /*seconds*/
#define LOW_VOLT_THRESHOLD	2000000
#define HIGH_VOLT_THRESHOLD	4200000
#define ADC_SAMPLE_COUNT	120 /*45s*/
...
//    printk(KERN_INFO "data->voltage_uV %d", data->voltage_uV);
    for (i = 0; i < tableSize; i++) {
    	
		if ((data->voltage_uV) >= (pTable[i].voltage)) {
			//printk(KERN_INFO "batty capability%d", pTable[i].percent);
			return	pTable[i].percent;

			//return pTable[0].percent;
		}
    }
...
extern u32 adc081c021_read_adc(void);
...
u32 calibration_voltage(struct max8903_data *data)
{
	...
	//printk("first delay %d\n", data->first_delay_count);

	if (data->first_delay_count <= 2) {
		voltage_data = adc081c021_read_adc()-offset_discharger;
		goto end;
	}
	for (i = 0; i < ADC_SAMPLE_COUNT; i++) {
			if (data->charger_online == 0 && data->usb_charger_online == 0) {
				/* ADC offset when battery is discharger*/
				volt[i] = adc081c021_read_adc()-offset_discharger;
				msleep(1000);
			} else {
					if (data->charger_online == 1)
						volt[i] = adc081c021_read_adc()-offset_charger;
					else if (data->usb_charger_online == 1)			
						volt[i] = adc081c021_read_adc()-offset_usb_charger;
...
static void max8903_battery_update_status(struct max8903_data *data)
{
	...
	if (!data->pdata->feature_flag) {
		...
		//printk(KERN_INFO "data->percent %d\n", data->percent);
...
static void max8903_battery_work(struct work_struct *work)
{
	...
	max8903_charger_update_status(data);
	max8903_battery_update_status(data);
//	printk(KERN_INFO "battery voltage: %4d mV\n" , data->voltage_uV);
//	printk(KERN_INFO "charger online status: %d\n" , data->charger_online);
//	printk(KERN_INFO "battery status : %d\n" , data->battery_status);
//	printk(KERN_INFO "battery capacity percent: %3d\n" , data->percent);
//	printk(KERN_INFO "data->usb_in: %x , data->ta_in: %x \n" , data->usb_in, data->ta_in);
...
static __devinit int max8903_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "*****************************load max8903 ok!");
	return 0;
err_psy:
