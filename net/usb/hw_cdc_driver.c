...
#else
static inline u16 get_unaligned_le16_e(const void *p)
{
	return le16_to_cpup((__le16 *)p);
}

static inline u32 get_unaligned_le32_e(const void *p)
{
	return le32_to_cpup((__le32 *)p);
}

static inline void put_unaligned_le16_e(u16 val, void *p)
{
	*((__le16 *)p) = cpu_to_le16(val);
}

static inline void put_unaligned_le32_e(u32 val, void *p)
{
...
#define INIT_NDP16_OPTS {					\
		...
		.nth_size = sizeof(struct usb_cdc_ncm_nth16_e),	\
		.ndp_size = sizeof(struct usb_cdc_ncm_ndp16_e),	\
		.ndplen_align = 4,				\
		.dgram_item_len = 1,				\
		.block_length = 1,				\
		.fp_index = 1,					\
...
#define INIT_NDP32_OPTS {					\
		...
		.nth_size = sizeof(struct usb_cdc_ncm_nth32_e),	\
		.ndp_size = sizeof(struct usb_cdc_ncm_ndp32_e),	\
		.ndplen_align = 8,				\
		.dgram_item_len = 2,				\
		.block_length = 2,				\
		.fp_index = 2,					\
...
static inline void put_ncm(__le16 **p, unsigned size, unsigned val)
{
	switch (size) {
	case 1:
		put_unaligned_le16_e((u16)val, *p);
		break;
	case 2:
		put_unaligned_le32_e((u32)val, *p);
...
static inline unsigned get_ncm(__le16 **p, unsigned size)
{
	...
	switch (size) {
	case 1:
		tmp = get_unaligned_le16_e(*p);
		break;
	case 2:
		tmp = get_unaligned_le32_e(*p);
		break;
	default:
		BUG();
	}
...
#define get_ncm_le16(p)				\
	({ __le16 val = get_unaligned_le16_e(p); p += 1; val; })

#define get_ncm_le32(p)				\
	({ __le32 val = get_unaligned_le32_e(p); p += 2; val; })

#define put_ncm_le16(val, p)				\
	({ put_unaligned_le16_e((val), p); p += 1; })

#define put_ncm_le32(val, p)				\
	({ put_unaligned_le32_e((val), p); p += 2; })
 ...
 */
#define NCM_NTB_MIN_OUT_SIZE		(sizeof(struct usb_cdc_ncm_nth16_e) \
	+ sizeof(struct usb_cdc_ncm_ndp16_e) + 2 * NCM_NDP16_ENTRY_LEN)
...
struct ncm_ctx {
	struct usb_cdc_ncm_desc_e *ncm_desc;
	//struct usbnet *unet;
...
for(i=0; i<3; i++)
{
	...
	if (ctx->rx_max_ntb > NCM_NTB_HARD_MAX_IN_SIZE) {
		...
		ctx->rx_max_ntb = NCM_NTB_HARD_MAX_IN_SIZE;
		put_unaligned_le32_e(ctx->rx_max_ntb, b);
