...
int pmic_write_brightness(int addr, int val)
{	
	int ret ;
	...
	ret  = pfuze_reg_write(x_pfuze, addr, val);
	...
	return 0;
err:
	printk(KERN_INFO "pmic write ERR\n");
	return -1;
}
EXPORT_SYMBOL_GPL(pmic_write_brightness);
		       ...
		       const struct i2c_device_id *id)
{
	...
	/*write 2.8v to vgen3*/
	printk(KERN_INFO "write 2.8v to vgen3 for camera\n");
	pfuze_reg_write(x_pfuze, 0x6e, 0x1a);
	/*write 1.35v to sw3a/b*/
	pfuze_reg_write(x_pfuze, 0x3c, 0x26);
	pfuze_reg_write(x_pfuze, 0x43, 0x26);
	pfuze_unlock(mc_pfuze);
