...
#define GPIO_NFC_WAKEUP	IMX_GPIO_NR(3, 0)
#define GPIO_NFC_IRQ	IMX_GPIO_NR(3, 15)
...
void wiresendcommand(unsigned char* cmd, unsigned char cmdlen)
 {
	...
	udelay(2);                // or whatever the delay is for waking up the board
	...
	if (ret != 7 + cmdlen) {
		printk(KERN_INFO "FAIL RO SEND CDM\n");
	}
} 
...
static ssize_t pn532_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
	...
	if (success) {
		...
		printk(KERN_INFO "UID Length: ");
		printk(KERN_INFO "%d", uidLength);
		printk(KERN_INFO " bytes");
